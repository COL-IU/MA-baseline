...
public class PttParser{
    ...
    public Hashtable<Integer, Gene> parseAndReturnStartHash(String ptt, String ct){
	Hashtable<Integer, Gene> locus2Gene = new Hashtable<Integer, Gene>();
	BufferedReader br = null;
	String curline = "";
	try{
	    br = new BufferedReader(new FileReader(ptt));
		...
		if(!inFlag){
		    ...
		    String[] tokens = curline.split("\\t");
		    int[] stend = getStartEnd(tokens[0]);
		    locus2Gene.put(new Integer(stend[0]), new Gene(ct, stend[0], stend[1], (tokens[1].equals("+") ? true : false) , tokens[4]));
		}
	    }
	    br.close();
	}catch(IOException ioe){
    ...
    public Hashtable<String, Gene> parseAndReturnGeneNameHash(String ptt, String ct){
	Hashtable<String, Gene> locus2Gene = new Hashtable<String, Gene>();
	BufferedReader br = null;
	String curline = "";
	try{
		...
		if(!inFlag){
		    ...
		    String[] tokens = curline.split("\\t");
		    int[] stend = getStartEnd(tokens[0]);
		    locus2Gene.put(tokens[4], new Gene(ct, stend[0], stend[1], (tokens[1].equals("+") ? true : false) , tokens[4]));
		}
	    }
	    br.close();
	}catch(IOException ioe){
    ...
    public ArrayList<Gene> parseSpecial(String ptt, String ct){
	ArrayList<Gene> list = new ArrayList<Gene>();
	BufferedReader br = null;
	String curline = "";
	try{
		...
		if(!inFlag){
		    ...
		    String[] tokens = curline.split("\\t");
		    int[] stend = getStartEnd(tokens[0]);
		    list.add(new Gene(ct, stend[0], stend[1], (tokens[1].equals("+") ? true : false) , tokens[4], curline) );
		}
	    }
	    br.close();
	}catch(IOException ioe){
